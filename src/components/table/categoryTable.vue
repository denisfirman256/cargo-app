  ...
  <section>
    <div class="my-4">
      <div class="d-flex align-items-center justify-content-between mb-3">
        ...
        <router-link class="btn btn-primary" :to="{name: 'Add_Category'}">Add Data</router-link>
      </div>
      <!-- Table -->
      <b-table 
        striped 
...
export default {
  ...
  methods: {
    // GetData
    getDataCategory(){
      axios.get('/api/v1/category_stuff', {headers: {Authorization : 'Bearer ' + localStorage.getItem('token')}}).then((response) => {
    ...
    deleteData(id){
      if (confirm('Apakah Ingin Dihapus')) {
        axios.delete('/api/v1/category_stuff/'+ id, {headers: {Authorization : 'Bearer ' + localStorage.getItem('token')}}).then((response) => {
            // Notification Success
            this.$notify({
